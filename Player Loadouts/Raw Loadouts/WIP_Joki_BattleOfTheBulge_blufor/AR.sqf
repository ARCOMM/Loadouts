this forceAddUniform "MNP_CombatUniform_USMC_arctic";
for "_i" from 1 to 5 do {this addItemToUniform "ACE_fieldDressing";};
this addItemToUniform "ACE_Flashlight_XL50";
this addItemToUniform "ACE_morphine";
this addItemToUniform "ACE_tourniquet";
this addItemToUniform "ACE_Fortify";
for "_i" from 1 to 2 do {this addItemToUniform "SmokeShell";};
for "_i" from 1 to 2 do {this addItemToUniform "CUP_HandGrenade_M67";};
this addVest "CUP_V_PMC_CIRAS_Winter_Patrol";
for "_i" from 1 to 4 do {this addItemToVest "hlc_100rnd_762x51_M_MG3";};
this addBackpack "MNP_B_WB_KB";
for "_i" from 1 to 6 do {this addItemToBackpack "hlc_100rnd_762x51_M_MG3";};
this addHeadgear "MNP_Helmet_USMC_arctic";
this addWeapon "hlc_lmg_MG3KWS_g";
this addPrimaryWeaponItem "optic_aco";
this addPrimaryWeaponItem "acc_flashlight";
this linkItem "ItemMap";
this linkItem "ItemCompass";
this linkItem "ItemWatch";
this addGoggles "G_Balaclava_blk";